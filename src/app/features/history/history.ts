...
export class History implements OnInit {
  sites: Site[] = [];
  selectedIds: number[] = [];
  selectedMetric: 'co2Total' | 'co2Energy' | 'co2Materials' | 'co2Parking' =
    'co2Total';
  ...
  async loadData() {
    ...
    this.error = '';
    try {
      this.sites = await this.siteService.loadSites();
  ...
  async rebuild() {
    ...
    for (const id of this.selectedIds) {
      let history = await this.siteService.getHistory(id);
    ...
    this.xLabels = months
      .map((m, i) => ({
        label: m.slice(0, 7),
        x: PAD.left + (i / (months.length - 1 || 1)) * this.innerW,
      }))
      ...
      const points: LinePoint[] = entries.map((e, i) => {
        ...
        return {
          ...
          y:
            PAD.top +
            this.innerH -
            ((val - minVal) / (maxVal - minVal)) * this.innerH,
          value: val,
          month: e.month,
        };
      });
